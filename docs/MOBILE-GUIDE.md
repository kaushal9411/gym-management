...
- **Member forgot/reset-password (Prompt 84) — request half only, plus staff-side parity with web's Prompt 83**: `forgot_password_screen.dart` (shared with staff) already called `MemberAuthRepository.forgotPassword({memberId})` before this prompt — the "request a reset link" half was already done. Added the missing "consume the token" half: `MemberAuthRepository.resetPassword({token, password})` + a new `MemberResetPasswordScreen` (`/member/reset-password`, reachable from the forgot-password confirmation screen's "Have a reset code? Enter it here" link). **This app has no deep-link handling at all** (no `app_links`/`uni_links`, no platform manifest wiring, no `main.dart` link listener) — a member tapping the emailed `https://.../portal/reset-password?token=...` link opens it in a browser, not the app, so the new screen has the user paste the code/link by hand (`_extractToken` pulls the `token` query param out if a full URL is pasted). Also added the staff-side counterpart to web's Prompt 83: `GymMember.portalStatus` (plain nullable String, matching this model's existing `status`/`gender`/`bloodGroup` convention, not a new enum), `MemberRepository.sendPortalPasswordReset()`, and `member_detail_screen.dart`'s `_buildPortalCard` now branches on it exactly like web (Enable/Resend/Send reset link/Suspended + an `AppPill` status badge) instead of showing "Enable portal access" unconditionally. **Not verified by tooling**: Flutter SDK is not installed on this dev machine (`flutter`/`dart` not on PATH, no `.dart_tool`, `pubspec.lock` present but never resolved) — `flutter analyze`/build/device run were all unavailable; verified only via careful manual review against the real widget APIs (`AppPill`, `AppLabeledField`, `AppButton`) already used elsewhere in the same files. Run `flutter analyze` and a real device pass before trusting this further.
