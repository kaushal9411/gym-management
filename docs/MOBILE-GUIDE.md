...
Four screens intentionally have **no** design frame of their own and exist as minimal glue: the Trainer's workout/diet **plan picker** lists (the design jumps straight from "Assign plan" to an already-open plan), the plan **create** forms, and the **Payments list** (`PaymentsScreen`) — frame 14 "Payment detail" is drawn with no navigation source, so the list was added beside Income/Expenses in the Menu's Finance section to reach it.
