...
export class ExpenseController {
  async list(req: Request, res: Response): Promise<void> {
    sendSuccess(res, await serviceFor(req).list(req.query as unknown as ListExpensesQuery));
  }
  ...
  async exportCsv(req: Request, res: Response): Promise<void> {
    const csv = await serviceFor(req).exportCsv(req.query as unknown as Partial<ListExpensesQuery>);
    res
      .status(200)
  ...
  async exportExcel(req: Request, res: Response): Promise<void> {
    const buffer = await serviceFor(req).exportExcel(req.query as unknown as Partial<ListExpensesQuery>);
    res
      .status(200)
