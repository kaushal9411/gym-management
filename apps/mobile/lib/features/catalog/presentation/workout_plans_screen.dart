...
/// Design frame "7b. Workout plans" — search + status filter + roster,
/// matching web's `/workout-plans` list. Tapping a card opens the same
/// create/edit form in edit mode; the "⋯" menu mirrors web's row dropdown
/// (Duplicate/Activate-Deactivate/Delete/Restore). Real plan creation also
/// still exists via the Trainer's own per-client flow (My Clients → Assign
/// Plan) — this screen is the second, plan-library-first entry point web
/// has, not a replacement for that one.
...
class _WorkoutPlansScreenState extends State<WorkoutPlansScreen> {
  ...
  late final _cubit = PaginatedListCubit<WorkoutPlanSummary>(
    (page) => getIt<WorkoutPlanRepository>().list(
      page: page,
      limit: 50,
  ...
  Widget build(BuildContext context) {
    return BlocProvider<PaginatedListCubit<WorkoutPlanSummary>>.value(
      value: _cubit,
      child: Scaffold(
        ...
        appBar: AppBar(
          backgroundColor: AppColors.bg,
          elevation: 0,
          title: BlocBuilder<PaginatedListCubit<WorkoutPlanSummary>,
              PaginatedListState<WorkoutPlanSummary>>(
            builder: (context, state) {
              final count = state is PaginatedListLoaded<WorkoutPlanSummary>
              ...
              child: Center(
                child: Container(
                  ...
                  child: IconButton(
                    ...
                    onPressed: () => context
                        .push(AppRoutes.workoutPlanForm)
                        .then((_) => _cubit.load()),
                  ),
                ),
              ),
    ...
    return Padding(
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          onTap: () => context
              .push(AppRoutes.workoutPlanForm, extra: plan.id)
