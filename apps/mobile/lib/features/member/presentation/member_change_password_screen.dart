...
/// Profile → "Change password" — mirrors the staff plane's
/// `ChangePasswordScreen` exactly, just against `POST /portal/change-password`
/// instead of `PATCH /auth/change-password`. Until this screen, the member
/// plane had no way to change a password while signed in at all — only the
/// logged-out "Forgot password" email-reset flow. Revokes every other
/// active session on success; this device stays signed in.
...
class _MemberChangePasswordScreenState
  ...
  Future<void> _submit() async {
    ...
    try {
      await getIt<MemberPortalRepository>().changePassword(
