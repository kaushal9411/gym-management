...
**Before exploring any code, read these docs — they replace codebase re-exploration:**

1. `docs/PROJECT-STATE.md` — what's built (prompt history), apps/ports, docker services, dev commands, test credentials, known dev-only caveats.
...
4. `docs/MOBILE-GUIDE.md` — apps/mobile (Flutter) state rules (SessionCubit + PaginatedListCubit, the IndexedStack tab-cache trap), the two auth planes, role shells, design-system/role-palette rules, backend traps (attendance scoping, whole-array PATCHes), adb workflow, and the full list of deliberately-dropped design elements.
...
- Extend the established architecture — never redesign it. Mirror an existing module (backend: `modules/branches` simplest, `modules/authentication` fullest; frontend: `features/iam` newest; mobile: `features/trainer` newest and fullest).
- **Mobile only ever renders real backend data.** If the design shows something no endpoint backs, drop it and record why in the screen's doc comment — never fake it. Check the API before concluding something is unbacked.
